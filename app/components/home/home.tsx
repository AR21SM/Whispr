...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      tealBands.forEach((band, i) => {
        ...
        (band as HTMLElement).style.transform = `translateX(${position}%) rotate(-3deg)`;
        (band as HTMLElement).style.opacity = `${0.9 - (Math.abs(position) / 200)}`;
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white relative overflow-hidden mt-">
      ...
      <nav className="relative z-10 px-8 py-6 flex justify-between items-center">
        ...
        <button className="bg-indigo-600 mt-12  text-white rounded-lg px-6 py-2 font-bold hover:bg-indigo-700 transition-colors shadow-lg hover:shadow-xl transform hover:-translate-y-1">
          Connect Wallet
        </button>
      </nav>
      ...
      <div ref={heroRef} className="px-8 py-12 md:py-10 flex flex-col md:flex-row items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="md:w-1/2 z-10">
          ...
          <div className="mb-12 flex space-x-4">
            <button className="bg-indigo-600 text-white rounded-lg px-8 py-3 font-bold shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300">
              Submit Report
            </button>
